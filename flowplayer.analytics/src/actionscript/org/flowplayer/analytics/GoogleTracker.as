    ...
    public class GoogleTracker extends Sprite implements Plugin {
        ...
        private function doTrackEvent(eventName:String, trackViewDuration:Boolean = false, event:ClipEvent = null):void {
            ...
            try {
                ...
                log.debug("Tracking " + eventName + "[" + (clip.completeUrl.split("?")[0] + (clip.isInStream ? ": instream" : "")) + "] : " + time + " on page " + getCategory());
                if (_tracker.isReady()) {
                    _tracker.trackEvent(getCategory(), eventName, clip.completeUrl.split("?")[0] + (clip.isInStream ? ": instream" : ""), time);
                }
            } catch (e:Error) {
